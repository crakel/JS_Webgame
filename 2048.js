...
window.addEventListener('mouseup', function(이벤트) {
    ...
    switch (방향) {
        case '왼쪽':
            ...
            데이터.forEach(function(열데이터, i) { //데이터처리 
                열데이터.forEach(function(행데이터, j) {
                    if (행데이터) {
            ...
            [1, 2, 3, 4].forEach(function(열데이터, i) { //그려줄때
        ...
        case '위':
            ...
            데이터.forEach(function(열데이터, i) { // forEach(function(element, index, array)) => element에 대하여 반복 (for _ in) index는 0부터 1씩증가
                열데이터.forEach(function(행데이터, j) {
                    if (행데이터) {
