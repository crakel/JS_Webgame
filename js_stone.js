...
var 턴 = true; // true면 내턴, false면 상대 턴

function 덱에서필드로(데이터, 내턴) {
    var 객체 = 내턴 ? 나 : 상대; // 3항 연산자 => 조건 ? 참 : 거짓
    var 현재코스트 = Number(객체.코스트.textContent);

    if (현재코스트 < 데이터.cost) {
        return 'end';
    }
    var idx = 객체.덱data.indexOf(데이터); // findIndex -> 배열의 첫번째 인덱스 반환
...
function 화면다시그리기(내화면) {
    var 객체 = 내화면 ? 나 : 상대; // 3항 연산자 => 조건 ? 참 : 거짓
    필드다시그리기(객체);
    덱다시그리기(객체);
    영웅다시그리기(객체);
}
...
function 턴액션수행(카드, 데이터, 내턴) {
            ...
            // 적군카드면서 아군 카드가 선택되어있고 턴이 끝난카드가 아니면
...
function 카드돔연결(데이터, 돔, 영웅) {
    var 카드 = document.querySelector('.card-hidden .card').cloneNode(true); // 기존 태그 내부까지 복사
...
function 상대덱생성(개수) {
    for (var i=0; i < 개수; i++){
        상대.덱data.push(카드공장(false, false)); // 객체를 배열에 집어넣는다
    }
    덱다시그리기(상대);
}
...
턴버튼.addEventListener('click', function() {
    ...
    필드다시그리기(객체);
    영웅다시그리기(객체); 
    턴 = !턴; // a = !a 는 true->false, flase->true
